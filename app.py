...
STRINGS = {
    ...
    "help_read_fail":     {"en": "the voice could not read this now",
                           "hr": "glas ovo sada ne može pročitati"},
...
def nav_tabs():
    ...
    # THE VR TAB WENT WITH HUME. A stored active_tab of "vr" falls
    # through to the default rather than blanking the page —
...
def kt_verdict(provider_id: str, key: str, secret: str = ""):
    ...
    try:
        # THE HUME PAIR BRANCH IS GONE WITH HUME. Every provider left
        # tests the same way — one key, one real piece of work.
...
def _match_voice_to(lang: str):
    ...
    if lang not in VOICES_BY_LANG:
        lang = "hr"
    # ONE CAST NOW. The speechify branch went with the engine; every
    # voice left is an Edge voice with a language, or a Gemini voice
    # which is not published per-language at all.
    current = st.session_state.get("voice", "Gabrijela")
...
def _voice_row_synth_only(engine, sp_ring_talk):
    ...
    """
def _voice_row(engine, sp_ring_talk=None):
    """The voices, ALWAYS on screen — writing or playing.

    THE SPEECHIFY BRANCH WENT WITH THE ENGINE. Two casts left: Edge's
    four, as buttons, and Gemini's thirty behind a radio and one list.
    `sp_ring_talk` is kept in the signature and ignored, so the two call
    sites did not have to change in the same commit as the deletion.
    """
...
# working deck would have been two answers to one question.
    ...
    # HELP YOU CAN LISTEN TO. Baba, 25.8.2026: "add in the help file to
    ...
    _hc1.button(t("tr_voice_f"), key="help_g_f",
                type="primary" if _hg == "F" else "secondary",
                use_container_width=True,
                on_click=lambda: st.session_state.update({"help_gender": "F"}))
    _hc2.button(t("tr_voice_m"), key="help_g_m",
                type="primary" if _hg == "M" else "secondary",
                use_container_width=True,
